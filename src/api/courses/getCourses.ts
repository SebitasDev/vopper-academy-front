import axios from "axios";
import {Course, ResponseApi} from "../../interfaces/apiInterfaces.ts";

const URL_BASE = "https://back-flqu.onrender.com/api"

export const getCourses = async (): Promise<ResponseApi<Course[]>> =>{
    try {

        const response = await axios
            .get<ResponseApi<Course[]>>(`${URL_BASE}/courses`)

        if (response.data.success) {
            console.log('Cursos obtenidos:', response.data.data);
            return response.data;
        } else {
            console.error('Error al obtener los cursos:', response.data.errorMessage);
            return {
                data: [],
                success: false,
                errorMessage: response.data.errorMessage,
                statusCode: response.data.statusCode,
            };
        }

    }catch (error) {
        console.error('Hubo un error en la solicitud:', error);

        return {
            data: [],
            success: false,
            errorMessage: "Estamos trabajando en ello, por favor intenta más tarde.",
            statusCode: 500,
        };
    }
}